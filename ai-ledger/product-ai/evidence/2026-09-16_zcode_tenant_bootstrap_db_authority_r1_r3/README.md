...
- **Normal-hook normalization of the evidence text.** The repository's
  ...
  was hand-edited. `EVIDENCE_NORMALIZATION_AUDIT.txt` records the committed
  digests and proves the change is semantic-free: the report's committed bytes
  differ from the as-generated bytes by exactly one end-of-file newline
  (removing it reproduces the recorded as-generated digest
  `2a6a7344e5e202a8…`), the report is identical after JSON parsing, and 28/28
  cross-consistency checks between the committed report and the committed logs
  pass (suite rcs, every mutation's FAILED nodeids and named REDs, verify rcs,
  and the mutation-restore script digests matching the candidate blobs). The
  post-normalization report was re-validated: identity R1-R3, verdict PASS,
  MM8 7/7, secret scan 0, committed report sha256 `c6be35f2938f14dd…`.
