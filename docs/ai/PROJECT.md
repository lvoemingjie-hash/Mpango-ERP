...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R8 status: evidence checkpoint, NO PASS verdict** — R8 applies four precision corrections over R7: coreutils evidence now covers exactly setup.sh's externals (dirname/grep/mkdir/seq/sleep + chmod) with probe non-executable/non-zero fail-closed; `.env` UnicodeDecodeError → one fixed neutral error; precise asymmetric port contract (`target` exact int only; `published` exact int or ASCII `[0-9]+` string per Compose v2); a genuinely unique sentinel proven absent from every argv/log/stdout/stderr. setup.sh is byte-identical to `0eb24d88`; direct preflight matrix 114/114 both orders; executable harness 21/21 both orders zero skip/xfail; complete H7 suite 229/229 both orders; real Compose JSON through the committed helper = OK. R7 (`0eb24d88`) is superseded; the CTO cross-host reproduction is preserved as **187 collected / 174 passed / 13 failed**; the focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/VPS/native-setup.sh claim.
