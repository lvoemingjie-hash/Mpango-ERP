...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R10 status: evidence checkpoint, NO PASS verdict** — extremely narrow correction: only the manifest-parity test and the three evidence docs change (everything else byte-identical to R9). The coreutils non-zero-return test no longer relies on `shutil.which("false")` (host-fragile: 244/1 where `false` is absent); it now uses `sys.executable` (guaranteed to exist and launch, receives the shell probe as Python code → non-zero exit → exactly `RuntimeError("coreutils probe failed")`), making it deterministic and host-independent. The OSError, missing-coreutil and successful-probe tests are retained. Direct preflight matrix 129/129 both orders; executable harness 22/22 both orders zero skip/xfail; complete H7 suite 245/245 both orders in both file orderings, and 245/245 under a Git-Bash-stripped PATH. R9 (`b495eb4a`) is `SUPERSEDED_BY_H7_R10` (R9 host-fragility 244/1 recorded truthfully); the CTO cross-host reproduction is preserved as **187 collected / 174 passed / 13 failed**; the focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/VPS/native-Linux-success/merge claim.
