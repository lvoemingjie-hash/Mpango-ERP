...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R12 status: evidence checkpoint, NO PASS verdict** — standalone Compose probe repair: candidate selection now uses `version` probes ONLY (the R11 premature `docker-compose config --format json` probe, which ran without `--env-file` and silently rejected a valid standalone Compose, is removed); the real capability checks run through the same `COMPOSE=(<candidate> --env-file "$BACKEND_ENV")` array. Structural guard rejects any `config` before the `COMPOSE=(` line. New authentic standalone harness (docker compose plugin hidden; standalone fake whose operations succeed ONLY with `--env-file` before the subcommand): GREEN completion + three mutation REDs (env-file removed / after subcommand / premature probe restored) all fail closed. Direct preflight 133/133 both orders; harness 27/27 both orders zero skip/xfail; complete H7 suite 254/254 both orders in both file orders. R11 (`849f31ca`) is `SUPERSEDED_BY_H7_R12`; accepted evidence: Kilo R10-V1 `7d53d6a5`, Lubuntu R10-V2 STOP `e073ded8`. The focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/merge claim.
