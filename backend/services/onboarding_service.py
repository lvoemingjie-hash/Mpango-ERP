...
async def create_signup_registration(
    *,
    ...
    # F-A: password is optional/deprecated. Validate the policy ONLY when a
    # legacy client still sends one (fail fast on garbage); it is never stored.
    ...
    if idempotency_key_hash is not None:
        ...
        if existing_for_key is not None:
            if existing_for_key.request_fingerprint_hash != fingerprint_hash:
                # R1-R1 cross-version compatibility: registrations written by
                # the pre-R1-R1 code carry a fingerprint computed WITH the
                # (now deprecated) password field. When the replaying request
                # itself carries a legacy password, also accept that legacy
                # fingerprint format so old clients can safely retry with the
                # same key and original payload instead of a spurious 409.
                legacy_match = (
                    request.password is not None
                    and existing_for_key.request_fingerprint_hash
                    == _request_fingerprint_hash_with_password(
                        request, owner_email, settings
                    )
                )
                if not legacy_match:
    ...
    registration = TenantRegistration(
        ...
        # F-A: password_hash stays NULL for new registrations. The single
        # customer password is created exclusively by the later
        # setup-credential step (owner users.password_hash). The nullable
        # column and terminal-state cleanup logic are retained for
...
def _request_fingerprint_hash_with_password(
    request: SignupRequest, owner_email: str, settings: Settings
) -> str:
    """Pre-R1-R1 fingerprint format (password included).

    Retained ONLY for idempotent-replay compatibility with registrations
    written by older code; never used to create new fingerprints.
    """
...
def _request_fingerprint_hash(
    ...
    # F-A: password is deliberately EXCLUDED from the canonical fingerprint.
    # Rationale: the field is deprecated and never stored, so it must not be
    # part of signup identity. Exclusion keeps the fingerprint deterministic
    # for passwordless requests AND lets a legacy client replay the same
    # idempotency key with a different (discarded) password without a
