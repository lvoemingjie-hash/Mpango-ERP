...
- **Normal-hook normalization of the evidence text.** The repository's
  ...
  was hand-edited. `EVIDENCE_NORMALIZATION_AUDIT.txt` records, per file, the
  as-generated digest and the committed digest and proves the change is
  semantic-free: `harness_report.json` is identical after JSON parsing
  (`json.loads(raw) == json.loads(committed)`), and every other file is
  identical after per-line right-trimming and trailing-blank-line removal.
  This is normalization only — the machine-generated verdict, identities,
  counts and assertions are unchanged, and the post-normalization report was
  re-validated (identity R1-R3, verdict PASS, MM8 7/7, secret scan 0,
  committed report sha256 `c6be35f2938f14dd…`).
