"""S4-E stock reservation lifecycle invariants.

These tests use real tenant DB sessions and real order route handlers to verify
quantity_on_hand, quantity_reserved, and available stock across confirm, cancel,
fulfill, and return flows.
"""
...
async def _create_order(
    ...
    *,
    ...
    order = Order(
        ...
        status=status,
        total_amount=total,
        notes="S4-E stock reservation lifecycle invariant",
    )
    order.items = [
        OrderItem(
...
@pytest.mark.asyncio
async def test_confirm_insufficient_available_rolls_back_order_and_stock(async_session):
    await _prepare_inventory_schema(async_session)
    sku = await _create_sku_with_stock(
        async_session, sku_code="S4E-CONFIRM-SHORT", on_hand=Decimal("2.00")
    )
    order = await _create_order(
        async_session,
        status=OrderStatus.DRAFT,
        items=[("S4E-CONFIRM-SHORT", 3, Decimal("25.00"))],
    )
    sku_id = sku.id
    order_id = order.id
...
async def test_concurrent_confirm_does_not_over_reserve_or_accept_excess_demand(async_session):
    ...
    results = await asyncio.gather(
        _confirm_in_independent_session(
            order_id=order_a_id, tenant_schema=tenant_schema, tenant_id=tenant_id
        ),
        _confirm_in_independent_session(
            order_id=order_b_id, tenant_schema=tenant_schema, tenant_id=tenant_id
        ),
    )
...
@pytest.mark.asyncio
async def test_cancel_releases_reservation_exactly_once(async_session):
    await _prepare_inventory_schema(async_session)
    sku = await _create_sku_with_stock(
...
@pytest.mark.asyncio
async def test_fulfillment_consumes_reservation_and_deducts_on_hand(async_session):
    await _prepare_inventory_schema(async_session)
    sku = await _create_sku_with_stock(
...
@pytest.mark.asyncio
async def test_fulfillment_without_reservation_keeps_reserved_non_negative(async_session):
    await _prepare_inventory_schema(async_session)
    sku = await _create_sku_with_stock(
        async_session, sku_code="S4E-DIRECT-PAID", on_hand=Decimal("10.00")
    )
    order = await _create_order(
        async_session,
        status=OrderStatus.PAID,
        items=[("S4E-DIRECT-PAID", 3, Decimal("25.00"))],
    )
