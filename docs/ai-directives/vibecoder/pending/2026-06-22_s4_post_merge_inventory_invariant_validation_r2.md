Directive-ID: s4-post-merge-inventory-invariant-validation-r2-2026-06-22
...
Validation-Scope: S4 post-merge order fulfillment inventory invariant validation, R2 DB-capable retry
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-22_s4_post_merge_inventory_invariant_validation_r2.md

# S4 Post-Merge Inventory Invariant Validation R2
...
R2 correction:
- R1 used an incorrect full commit hash even though the short hash was correct.
- R1 also assumed PostgreSQL was listening on `127.0.0.1:5432`. R2 must start
  or discover the Docker Compose PostgreSQL/Redis services, verify PostgreSQL
  credentials from inside the container, run migrations, then execute tests.
