/**
 * P22-C: Controlled Execution nav + guard wiring tests.
 *
 * Verifies the additive wiring only:
 *   - the Sidebar shows the "Controlled Execution" link for an identity-only
 *     platform operator and hides it for everyone else (case 13)
...
describe('P22 controlled execution nav + guard', () => {
  it('P22-N01: Sidebar shows the Controlled Execution link for identity-only super_admin', () => {
    ...
    const link = screen.getByText('Controlled Execution').closest('a');
    expect(link).not.toBeNull();
    expect(link?.getAttribute('href')).toBe('/platform/controlled-execution');
  });

  it('P22-N02: Sidebar hides the Controlled Execution link for tenant-contextual super_admin', () => {
    ...
    expect(screen.queryByText('Controlled Execution')).not.toBeInTheDocument();
  });

  it('P22-N03: Sidebar hides the Controlled Execution link for non-platform users', () => {
  ...
  it('P22-N04: PlatformRoute admits identity-only super_admin to the console', () => {
    useAuthStore.setState({ user: identityOnly, accessToken: 'token' });
    render(
  ...
  it('P22-N05: PlatformRoute redirects tenant-contextual super_admin away from the console', () => {
    useAuthStore.setState({ user: tenantContextual, accessToken: 'token' });
    render(
      <MemoryRouter initialEntries={['/platform/controlled-execution']}>
        <Routes>
