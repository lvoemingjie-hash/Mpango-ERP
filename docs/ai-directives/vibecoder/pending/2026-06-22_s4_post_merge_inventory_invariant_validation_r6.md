Directive-ID: s4-post-merge-inventory-invariant-validation-r6-2026-06-22
...
Validation-Scope: S4 post-merge order fulfillment inventory invariant validation, R6 shell export fix
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-22_s4_post_merge_inventory_invariant_validation_r6.md

# S4 Post-Merge Inventory Invariant Validation R6
...
R6 correction:
- R5 passed DB readiness but failed under `set -u` because `TEST_DATABASE_URL`
  referenced `DATABASE_URL` in the same export command. R6 assigns URLs in
  separate statements.
