Directive-ID: s4-post-merge-inventory-invariant-validation-r4-2026-06-22
...
Validation-Scope: S4 post-merge order fulfillment inventory invariant validation, R4 compose mandatory-env retry
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-22_s4_post_merge_inventory_invariant_validation_r4.md

# S4 Post-Merge Inventory Invariant Validation R4
...
R4 correction:
- R3 reached Compose but Compose v1 parsed the backend service while starting
  only postgres/redis and required a mandatory signing env value. R4 provides a
  non-production runner-only value via shell name concatenation.
