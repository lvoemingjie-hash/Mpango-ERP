# DC-2T1-R1 Full-Suite Inventory Reconciliation Report

- **Date**: 2026-07-10
- **Task ID**: DC-2T1-R1 (Full-Suite Inventory Reconciliation)
...
|----------|-------|---|-----------|
| **A**: TEST_FIXTURE_ISOLATION | **591** | 86.0% | Shared DB/tenant/schema/event-loop pollution; pass in isolation |
| **B**: SKELETON_TEST_RETIRED | **19** | 2.8% | Missing deliverable docs (3 auth) + missing decision/contract docs (16 other) |
| **C**: STALE_TEST_CONTRACT | **8** | 1.2% | Model structure (4) + registry status (2) + redaction false positive (1) + event loop API (1) |
| **D**: CURRENT_PRODUCT_REGRESSION | **5** | 0.7% | Docstring leak, endpoint exposure, 500 on duplicate email, import leak, state mismatch |
| **E**: ENVIRONMENT_GATED_RUNTIME_TEST | **23** | 3.3% | Event loop (1) + live DB import by design (22) |
...
**Verification**: 591 + 19 + 8 + 5 + 23 + 11 + 30 = **687** ✓
...
|--------|-------|---|---|---|---|---|---|---|-------|
| Auth/Onboarding | 18 | 148 | 3 | 0 | 4 | 1 | 3 | 0 | 159 |
| Payment/Ledger/Order | 10 | 75 | 0 | 0 | 0 | 0 | 0 | 13 | 88 |
| Provisioning/Bootstrap | 4 | 48 | 0 | 0 | 1 | 0 | 0 | 1 | 50 |
| Security/Tenant Isolation | 5 | 27 | 0 | 0 | 0 | 0 | 7 | 0 | 34 |
| Platform/Infra/Integration | 25+ | 293 | 16 | 8 | 0 | 22 | 1 | 16 | 356 |
| **Total** | **62+** | **591** | **19** | **8** | **5** | **23** | **11** | **30** | **687** |
...
- **652 pollution tests** (591A + 3B + 1D + 23E + 16B + 8C + 1F + 16G = additional "other" genuine: 63, plus blocking genuine: 33, plus pollution: 591 = 687)
...
| 11 | test_u6i0_owner_credential_setup_contract.py | 1 | D | 1 | Docstring in `api/v1/auth.py` leaks "owner credential setup" — HIGH information disclosure |
...
| 17 | test_u6i5_owner_credential_setup_endpoint.py | 1 | D | 1 | Endpoint source code exposed in route registration — HIGH information disclosure |
| 18 | test_u6k_production_smtp_email_delivery.py | 4 | A | 4 | Email dev-sink capture polluted by prior module |
| 19 | test_u6k_production_smtp_email_delivery.py | 1 | D | 1 | Duplicate signup email returns 500 Internal Server Error instead of 202 — HIGH |
| 20 | test_users_roles_api.py | 23 | A | 23 | Users/roles table polluted by prior onboarding chain |
| | **Subtotal** | **151** | | **151** | A:141, B:3, D:3, E:1, F:3 |

### 3.2 Onboarding (2 files, 8F, 0E)

| # | File | F | Category | Count | Root Cause |
|---|------|---|----------|-------|------------|
| 21 | test_u6i6_onboarding_e2e_closeout.py | 1 | D | 1 | State mismatch: expected `'active'`, got `'email_verified'` — state machine stops one step early |
| 22 | test_u6l_email_verified_onboarding_orchestration.py | 7 | A | 7 | Orchestration chain polluted by prior provisioning/tests |
| | **Subtotal** | **8** | | **8** | A:7, D:1 |
...
### 3.4 Provisioning/Bootstrap (4 files, 50F, 0E)
...
| 36 | test_u6h1_tenant_provisioning_service_skeleton.py | 1 | D | 1 | `TenantProvisioningService` imported in auth signup service — cross-domain dependency leak |
...
### 4.1 Category A: TEST_FIXTURE_ISOLATION (293 tests)
...
### 4.3 Category C: STALE_TEST_CONTRACT (8 tests)
...
### 4.4 Category E: ENVIRONMENT_GATED_RUNTIME_TEST (22 tests)

| # | File | F | E | Total | Required Environment |
|---|------|---|---|-------|---------------------|
| 88 | test_u3b2_live_db_import_preview_validate.py | 0 | 14 | 14 | Live PostgreSQL with seed data; alembic schema alone insufficient |
| 89 | test_u3c_live_db_apply.py | 0 | 8 | 8 | Live PostgreSQL with seed data |
| | **E subtotal** | | | **22** | |

> **Note**: Per DC-2A-R2 Section 4.1, these files are classified as "NON_BLOCKING (live DB requirement by design)."

### 4.5 Category F: CONFIGURATION_DRIFT (1 test)

| # | File | F | Count | Root Cause | Fix |
|---|------|---|-------|-----------|-----|
| 90 | test_platform_p21_durable_approval_adapter_skeleton.py | 1 | 1 | Expects no migration after `020_durable_approval_store`, but `029` and `030` exist | Update `ALLOWED_DESCENDANTS` to `{"030_platform_backup_status_source.py"}` |

### 4.6 Category G: CONFIRMED_MIGRATION_GAP (16 tests)

| # | File | F | E | Count | Missing Migration/View | Fix |
|---|------|---|---|-------|------------------------|-----|
| 91 | test_s4g_migration_infrastructure_hardening.py | 5 | 0 | 5 | Migration 017 (`017_retailer_prices`): retailer_prices table structurally incomplete in `t_dev` tenant schema. Tests: `test_alembic_upgrade_head_creates_wide_version_table...`, `_widens_existing_...`, `test_migration_017_creates_retailer_prices_on_fresh_tenant_schema`, `_reconciles_compatible_...`, `_fails_closed_for_incompatible_...` | Fix migration 017 to add all columns/constraints/indexes |
| 92 | test_s6_2_materialized_views.py | 4 | 1 | 5 | `mv_sales_daily` materialized view does not exist. Tests: `test_mv_sales_daily_has_unique_index`, `_accessible_by_reporting_user`, `_staleness_then_refresh`, `_receivables_summary_is_realtime`, `_advisory_lock_prevents_double_refresh` | Create `mv_sales_daily` materialized view migration |
| 93 | test_s6_3_dashboard_api.py | 5 | 1 | 6 | Dashboard queries depend on `mv_sales_daily`. Tests: `test_query_builder_empty_mv_returns_zeros`, `_fetch_all_receivables`, `_fetch_kpi_summary`, `_fetch_time_series`, `_reporting_user_access`, `_cross_view_metric_raises` | Same as above |
| | **G subtotal** | | | **16** | | |
...
## 6. Detailed Analysis: CURRENT_PRODUCT_REGRESSION (5 tests)

| # | Test ID | Expected | Actual | Severity | Product Code Location | Secondary |
|---|---------|----------|--------|----------|----------------------|-----------|
| 1 | `test_u6i0::test_contract_locks_public_endpoint_disclosure_boundary` | No docstring mentioning "owner credential setup" in auth router | Docstring leaks endpoint name | HIGH | `api/v1/auth.py` | Also B (contract doc missing) |
| 2 | `test_u6i5::test_setup_credential_route_disclosure_boundary` | Endpoint source code not exposed in route registration | Source file path exposed | HIGH | `api/v1/auth.py` route registration | Also A (9 of 10 in file are pollution) |
| 3 | `test_u6k::test_duplicate_live_email_in_production_is_neutral_and_sends_no_extra_smtp` | Duplicate signup returns 202 (neutral) | Returns 500 Internal Server Error | HIGH | Auth signup handler | Also A (4 of 5 are pollution) |
| 4 | `test_u6h1::test_public_auth_routes_do_not_call_tenant_provisioning` | Auth signup does NOT import `TenantProvisioningService` | Import present — cross-domain leak | HIGH | Auth signup service | Also A (9 of 10 are pollution) |
| 5 | `test_u6i6::test_full_owner_onboarding_backend_chain` | Final onboarding state is `'active'` | Final state is `'email_verified'` | MEDIUM | Onboarding orchestration handler | Also A (7 of 8 in file are pollution) |

> **Note**: Each D-category test appears in a file where the majority of tests are A (pollution). The genuine regression is confirmed by isolation re-runs.
...
### WP-06: Product Regression Fixes

**Scope**: 5 D-category tests

| File | Tests | Action |
|------|-------|--------|
| api/v1/auth.py | — | (1) Remove docstring mentioning "owner credential setup" |
| api/v1/auth.py route registration | — | (2) Remove endpoint source code exposure |
| Auth signup handler | — | (3) Handle duplicate email → return 202 not 500 |
| Auth signup service | — | (4) Remove `TenantProvisioningService` import |
| Onboarding orchestration | — | (5) Fix state machine: ensure `'active'` not `'email_verified'` |

**Allowed-to-change**: Auth router module, signup handler, signup service, onboarding orchestration
**Forbidden-to-change**: Test files (tests should not be modified for product fixes)
**Verification**: Run isolation re-runs per domain (see DC-2A-R2 Section 6 commands)
**Completion criteria**: 0 failures in isolated re-runs; no new failures in full suite

### WP-07: Migration Gap — retailer_prices
...
**Allowed-to-change**: `alembic/versions/017_retailer_prices.py` (or new migration)
**Forbidden-to-change**: Test files, other migrations, product code
**Fix**: Ensure `017_retailer_prices` creates `t_dev.retailer_prices` with: `retailer_id` (FK), `sku_id` (FK), `price` (NUMERIC), `created_at`, `updated_at`, `is_deleted`, unique constraint (retailer_id+sku_id), check constraint (price > 0), indexes on retailer_id and sku_id
**Verification**: `pytest -v tests/test_payments_schema_contract.py tests/test_s4g_migration_infrastructure_hardening.py`
**Completion criteria**: 0 failures

### WP-08: Migration Gap — mv_sales_daily

**Scope**: 18 G-category tests (1 from provisioning + 5 from mat views + 6 from dashboard + 6 from s6_4 exports? No — s6_4 is A)
...
**Allowed-to-change**: New alembic migration creating `mv_sales_daily`
**Forbidden-to-change**: Test files, other migrations, product code
**Fix**: Create migration that adds `mv_sales_daily` materialized view with unique index
...
**Scope**: 23 E-category tests + 1 auth E test
...
**Completion criteria**: All 23 tests skipped (not failed) when environment unavailable
...
|----------|----|-------|--------|------|----------|
| **P0** | WP-06 | 5 | Medium | LOW (targeted fixes) | D: Product Regression |
...
| **P1** | WP-10 | 23 | Low | LOW (skip markers) | E: Env Gated |
...
The 591 A-category tests (TEST_FIXTURE_ISOLATION) represent test pollution that requires fixture isolation infrastructure changes. These are NOT batched into skip/xfail work packages. WP-09 addresses the highest-priority subset (60 business invariant tests).

**Remaining A-category tests (531)**: Platform, intake, auth, payment, provisioning tests that pass in isolation but fail in full suite. These require:
...
2. **5 genuine product regressions** (D) — all fixable with targeted changes to auth router/signup/orchestration code.
...
4. **591 pollution tests** (A) — confirmed by isolation re-runs; represent test infra debt, NOT product defects.
...
7. **8 stale contract tests** (C) — minor assertion updates.
8. **23 environment-gated tests** (E) — properly marked as integration/live-DB tests.

**No blocking issue that cannot be resolved by the 10 work packages defined above.**

**Pre-conditions for DC-2T implementation:**
- [ ] WP-06 (product regressions) merged
...
- bcrypt 4.0.1 / passlib 1.7.4 incompatibility (confirmed, NOT a product defect; doesn't manifest in DC-2A-R2 DB env)
...
- Test fixture isolation infrastructure (WP-09 + remaining 531 A-category tests — ongoing)

---

*Report generated by DC-2T1-R1 subagent. Evidence-first classification. No code modified.*
