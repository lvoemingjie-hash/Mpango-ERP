...
**Accepted readiness-debt merge:** `a6ef3aac0ab03615e9d70e08e504b9858baf61c5` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 bcrypt/dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite; H7-R3 (current) supersedes H7-R2, adding the `et-xmlfile==2.0.0` pin and fail-closed manifest parsers to reach exact normalized name/version parity between `requirements.txt` and Poetry's main-runtime lock set (name/version parity only; no deployment/Playwright/VPS/native-setup.sh claim).
