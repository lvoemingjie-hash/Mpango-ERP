...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R5-R6 status: evidence checkpoint, NO PASS verdict** — preflight extracted into stdlib-only `scripts/setup_preflight.py` (rendered Compose JSON from stdin, strict .env/URL/credential/port validation, `--post-install` core.config comparison, outputs only `OK`, never secrets); setup.sh pipes `compose config --format json` through it with `pipefail` and adds a CRLF fail-closed self-check; direct preflight matrix 76/76 both orders; executable harness 17/17 both orders zero skip/xfail; complete H7 suite 187/187 both orders; real Compose JSON piped through the committed helper = OK. R5-R5 (`abbbe32f`) is superseded; its CTO reproduction (Windows) is preserved as **11 collected / 4 passed / 7 failed**; the focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/VPS/native-setup.sh claim.
