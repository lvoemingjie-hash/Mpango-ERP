Directive-ID: s4-post-merge-inventory-invariant-validation-r7-2026-06-22
...
Validation-Scope: S4 post-merge order fulfillment inventory invariant validation, R7 strong runner signing value
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-22_s4_post_merge_inventory_invariant_validation_r7.md

# S4 Post-Merge Inventory Invariant Validation R7
...
R7 correction:
- R6 reached pytest import but the runner signing value was rejected by
  application config because it contained a weak substring. R7 keeps the same
  DB-capable validation path and uses a strong-looking runner-only signing
  value with no weak substring.
