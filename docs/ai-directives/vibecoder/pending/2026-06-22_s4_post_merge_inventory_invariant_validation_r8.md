Directive-ID: s4-post-merge-inventory-invariant-validation-r8-2026-06-22
...
Validation-Scope: S4 post-merge inventory invariant validation with corrected deterministic evidence labels
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-22_s4_post_merge_inventory_invariant_validation_r8.md

# S4 Post-Merge Inventory Invariant Validation R8
...
R8 correction:
- R7 workflow passed, but deterministic fallback report labels still used old
  U3D wording. R8 repeats the same product validation with corrected report
  labels from actual command output.
...
- PREFLIGHT: 5/5
- VALIDATION: 4/4
- App Import Smoke: U3C+S4 combined command passed, with no failed tests.
- Receivables Suite: inventory selection command passed, with no failed tests.
...
- Schema Skip Reasons: NONE, unless command 4 reports a real skip, which must fail the gate.
