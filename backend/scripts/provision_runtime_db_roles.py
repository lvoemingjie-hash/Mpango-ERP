...
class Provisioner:
    ...
    async def create_roles_and_database(self, app_password: str | None,
        ...
        try:
            # R1-R1 P0: ownership gate BEFORE any write.  The target database
            # is either ABSENT (phase 1 creates it owned by the migration
            # authority) or MUST already be owned by the migration authority.
            # Provisioning never adopts, seizes or re-owns an existing
            # database: a wrong-owner target (administrator-owned compose
            # init, third-party database, established deployment) refuses
            # here, before a single role, ownership or grant write.
            db_state = await admin.fetchrow(
                "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = $1) "
                "AS database_exists, "
                "COALESCE((SELECT pg_get_userbyid(datdba) FROM pg_database "
                "          WHERE datname = $1), '') AS owner",
                self.database,
            )
            if db_state["database_exists"] and                     db_state["owner"] != self.migrate_role:
                raise RuntimeError(
                    "provisioning refused with zero writes: target database "
                    f"{self.database!r} already exists and is owned by "
                    f"{db_state['owner']!r}, not the migration authority "
                    f"{self.migrate_role!r} - provisioning never adopts or "
                    "re-owns an existing database; the application database "
                    "must be absent (phase 1 creates it) or already owned by "
