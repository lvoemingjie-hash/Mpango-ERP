...
Directive-ID: product-dev-recovered-sprint-c-validation-r4-2026-05-19
Priority: HIGH
Created: 2026-05-19
Status: pending
Target-Branch: product-dev-recovered
Target-Commit: c0f80d7cf69d197877453375974c067223cf366e
Validation-Scope: product-dev-recovered post Sprint C promotion validation (R4 after v3.7 refactor)
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-05-19_product_dev_recovered_sprint_c_validation_r4.md

# Product Dev Recovered Sprint C Validation — R4

Objective:
Re-validate `origin/product-dev-recovered` after v3.7 runner refactor (evidence extraction moved to standalone Python helper).

What changed in v3.7:
- `parse_evidence_from_json` no longer contains inline Python via `python3 -c "..."`.
- Evidence extraction delegated to `scripts/runner/extract_leo_evidence.py` (standalone file).
- Eliminates bash quote/backslash/newline escaping issues that caused R3 early failure.
- `bash -n` now passes (previously failed due to inline Python syntax).
...
R4 Acceptance Criteria (runner gates):
...
If R4 fails, report failure phase ONLY:
- script syntax / helper parse / Leo timeout / evidence gate / report push / final gate
