...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R15-R2 status: evidence checkpoint, NO PASS verdict** — narrow test-only correction (setup.sh and all product files byte-identical to R12): the standalone harness's `docker-compose` fake is now included in the `chmod +x` set only when `standalone=True` (masked on MSYS, fatal on POSIX), the normal harness never attempts to chmod a file it does not create, and new fail-closed assertions prove the fakes exist and pass the selected Bash's `test -x` (POSIX/MSYS executability). Direct preflight 133/133 both orders; harness 29/29 both orders zero skip/xfail; complete H7 suite 256/256 both orders in both file orders. R12 (`db166b77`) is `SUPERSEDED_BY_H7_R13`; GitNexus note: `status` up-to-date at `db166b7` but local MCP `compare` misreports historical files — the precise `git diff` scope (R12 = 5 files) is authoritative. The focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/merge claim.
