...
import {
  loadIdentities, apiLogin, apiSelectTenant, apiGet, apiGetNoAuth, apiPost, loginRetailerPortal, ensureStockAndPrice,
} from './helpers';
...
test.describe('PW1-R1 Phase 5: isolation & negative paths', () => {
  ...
  test('cross-retailer: RB cannot read RA order by id (API isolation)', async () => {
    ...
    if (!raOrderId) {
      ...
      // PW1-R4-B F1: stock + retailer price via supported lifecycles so the
      // RA order create passes the real server-side validation.
      await ensureStockAndPrice(w1Token, {
        skuCode, skuId: sku.body.data?.id,
        retailerEmail: IDS.ra.email, quantity: 100, price: '10.00',
      });
  ...
  test('unauthenticated API access is rejected with structured 401 (no leak)', async () => {
    // PW1-R4-B F3: preserve the canonical taxonomy with DISTINCT assertions —
    // do NOT weaken to status-only.
    // (a) MISSING Authorization header -> UNAUTHENTICATED
    const missing = await apiGetNoAuth('/orders');
    expect(missing.status).toBe(401);
    expect(missing.body?.code, 'missing Authorization -> UNAUTHENTICATED').toBe('UNAUTHENTICATED');
    // (b) MALFORMED bearer token -> INVALID_TOKEN
    const malformed = await apiGet('/orders', 'not-a-real-token');
    expect(malformed.status).toBe(401);
    expect(malformed.body?.code, 'malformed token -> INVALID_TOKEN').toBe('INVALID_TOKEN');
    // Neither envelope may leak internals
  ...
  test('RB orders page contains no RA-created orders (page-level isolation)', async ({ page }) => {
    ...
    const sku5 = await apiPost('/skus', w1Token, { sku_code: skuCode, name: 'ISO5 SKU', unit: 'pcs' });
    expect(sku5.status, `SKU create (${JSON.stringify(sku5.body).slice(0, 200)})`).toBe(201);
    // PW1-R4-B F1: stock + retailer price via supported lifecycles so the
    // RA order create passes the real server-side validation.
    await ensureStockAndPrice(w1Token, {
      skuCode, skuId: sku5.body.data?.id,
      retailerEmail: IDS.ra.email, quantity: 100, price: '10.00',
    });
