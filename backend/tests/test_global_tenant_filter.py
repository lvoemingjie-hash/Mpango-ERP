...
def test_requires_tenant_context_for_orm_select():
    engine = _setup_db()
...
def test_orders_query_without_tenant_id_raises_tenant_context_missing_error():
    engine = _setup_db()

    with Session(engine) as session:
        # Simulate tenant schema being known while tenant_id is missing.
...
def test_ignore_tenant_escape_hatch_skips_enforcement():
    engine = _setup_db()
...
def test_run_as_system_bypass_skips_enforcement():
    engine = _setup_db()
...
def test_applies_tenant_id_filter_when_model_has_tenant_id_column():
    engine = _setup_db()
...
def test_ignore_tenant_skips_tenant_id_filtering():
    engine = _setup_db()
