"""S5-D5 -- structured payment ledger runtime invariant tests."""
...
async def _seed_confirmed_order(async_session, *, total: Decimal):
    ...
    await async_session.execute(
        text(
            ...
            VALUES (:tenant_id, :code, 'S5D5 Test Wholesaler', 'active', FALSE)
            ON CONFLICT (id) DO UPDATE
            SET status = 'active', is_deleted = FALSE, updated_at = now()
            """
        ),
        {"tenant_id": tenant_id, "code": f"S5D5{str(order_id).replace('-', '')[:8]}"},
    )
    await async_session.execute(
        text(
            """
            INSERT INTO public.retailers (id, phone, name, is_deleted)
            VALUES (:retailer_id, :phone, 'S5D5 Test Retailer', FALSE)
            ON CONFLICT (id) DO UPDATE
            SET is_deleted = FALSE, updated_at = now()
            """
        ),
        {"retailer_id": retailer_id, "phone": f"+1666{str(order_id).replace('-', '')[:10]}"},
    )
    await async_session.execute(
        text(
            """
...
def _assert_completed_payment_ledger_covered(snapshot) -> None:
    completed_total = Decimal(str(snapshot["completed_total"]))
    assert snapshot["completed_count"] > 0
    assert Decimal(str(snapshot["cash_debit"])) == completed_total
    assert Decimal(str(snapshot["receivable_credit"])) == completed_total
    assert Decimal(str(snapshot["settlement_sum"])) == Decimal("0.0000")
    assert snapshot["settlement_entry_count"] == 2


@pytest.mark.asyncio
async def test_full_cash_payment_completes_and_has_balanced_ledger(async_session):
    ...
    snapshot = await _payment_ledger_snapshot(async_session, order_id=order_id)
    assert response.data["status"] == "paid"
    assert snapshot["order_status"] == "paid"
    _assert_completed_payment_ledger_covered(snapshot)
    ...
    after_duplicate = await _payment_ledger_snapshot(async_session, order_id=order_id)
    assert after_duplicate == snapshot


@pytest.mark.asyncio
async def test_partial_cash_payment_only_gets_completed_ledger_coverage_on_final_payment(async_session):
    order_id, token = await _seed_confirmed_order(async_session, total=Decimal("100.00"))
    ...
    assert after_final["completed_count"] == 2
    _assert_completed_payment_ledger_covered(after_final)


@pytest.mark.asyncio
async def test_partial_transfer_payment_does_not_complete_without_ledger_coverage(async_session):
    order_id, token = await _seed_confirmed_order(async_session, total=Decimal("100.00"))
...
@pytest.mark.asyncio
async def test_full_transfer_payment_completes_and_has_balanced_ledger(async_session):
    order_id, token = await _seed_confirmed_order(async_session, total=Decimal("100.00"))
...
@pytest.mark.asyncio
async def test_credit_payment_is_pending_and_excluded_from_completed_ledger_invariant(async_session):
    order_id, token = await _seed_confirmed_order(async_session, total=Decimal("100.00"))
    ...
    credit_status = (
        await async_session.execute(
            text("SELECT status FROM payments WHERE order_id = :order_id AND method = 'credit'"),
            {"order_id": order_id},
        )
    ).scalar_one()

    assert response.data["status"] == "paid"
    assert credit_status == "pending"
    assert snapshot["completed_count"] == 0
    assert Decimal(str(snapshot["cash_debit"])) == Decimal("0")
    assert Decimal(str(snapshot["receivable_credit"])) == Decimal("0")
