Directive-ID: u3d-post-merge-validation-r3-2026-06-13
...
Validation-Scope: U3-D post-merge validation R3 with OpenClaw raw-output diagnostics
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-06-13_u3d_post_merge_validation_r3.md

# U3-D Post-Merge Validation R3

Objective:
Repeat U3-D VALIDATION_GATE after adding raw OpenClaw output capture. If Leo
still exits before executing commands, the failure report must contain the exact
OpenClaw CLI error instead of an opaque exit code.
...
- If OpenClaw exits before command execution, classify as `FAIL_RUNNER_INFRA` and preserve raw stderr in the report/log.
