#!/usr/bin/env python3
"""DC-11T0 deterministic backend test gate.
...
SECRET_KEY = hashlib.sha256(b"dc11t0-r4-deterministic-gate-secret").hexdigest()
...
def unique_prefix(label: str) -> str:
    ...
    return f"dc11t0-r4-{safe_label}-{timestamp}-{suffix}"
...
def run_alembic_checks(env: dict[str, str], artifacts_dir: Path, secrets: list[str]) -> dict[str, Any]:
    heads = run([sys.executable, "-m", "alembic", "heads"], env=env, secrets=secrets, timeout=120)
...
def run_test_schema_bootstrap(
    env: dict[str, str], artifacts_dir: Path, secrets: list[str]
) -> dict[str, Any]:
    """Create the canonical tenant schemas required by the full test suite."""
    outputs: list[str] = []
    schemas = ("t_test", "t_dev")
    for schema in schemas:
        proc = run(
            [sys.executable, "scripts/bootstrap_tenant_schema.py", schema],
            env=env,
            secrets=secrets,
            timeout=300,
        )
        outputs.append(redact(proc.stdout, secrets))
    (artifacts_dir / "tenant_bootstrap_output.txt").write_text(
        "\n".join(outputs), encoding="utf-8"
    )
    return {"schemas": list(schemas), "status": "ready"}
...
def command_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        summary["tenant_bootstrap"] = run_test_schema_bootstrap(
            env, artifacts_dir, secrets
        )
