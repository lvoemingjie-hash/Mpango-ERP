...
### WP-00: bcrypt/passlib Dependency Fix (R2 NEW)

**Scope**: Resolves D3 blocker + enables all password-dependent tests

| Action | Detail |
|--------|--------|
| Pin bcrypt | `bcrypt==4.0.1` in requirements (confirmed compatible with passlib 1.7.4 in DC-2A-R2) |
| Or migrate | Replace passlib with direct bcrypt 5.x usage (larger effort) |

**Allowed-to-change**: `requirements.txt`, `pyproject.toml`, or password utility module
**Forbidden-to-change**: Test files (tests should work after bcrypt fix)
**Verification**: `pytest tests/test_password_utils.py` — all 4 tests should pass
**Impact**: Unblocks D3 test, may stabilize additional auth chain tests affected by bcrypt crashes
**Priority**: **P0** (blocks D3, affects signup/login/credential flows)
...
|----------|----|-------|--------|------|----------|
| **P0** | WP-00 | 1+ | Low | LOW (pin bcrypt) | Dependency Fix (R2 NEW) |
...
**bcrypt/passlib incompatibility** (bcrypt 5.0.0 + passlib 1.7.4) is a **critical blocker** affecting ALL password-dependent operations. Must pin `bcrypt<4.1.0` before meaningful test execution. This is a dependency issue, not a product defect.

**No blocking issue that cannot be resolved by the 11 work packages defined above.**

**Pre-conditions for DC-2T implementation:**
- [ ] WP-00 (bcrypt pin) applied ← **R2 NEW: Critical prerequisite**
...
- bcrypt 4.0.1 / passlib 1.7.4 incompatibility (confirmed, NOT a product defect; doesn't manifest in DC-2A-R2 DB env) — **R2 UPDATE: Now tracked as WP-00, must pin bcrypt before any auth-dependent test execution**
