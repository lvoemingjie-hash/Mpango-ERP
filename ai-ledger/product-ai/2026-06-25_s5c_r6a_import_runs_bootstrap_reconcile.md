...
- **DDL**: Added `import_runs` `CREATE TABLE IF NOT EXISTS` to the `tables` list
  (line 593–617), mirroring migration 022 contract exactly: 21 columns including
...
- **Reconcile function**: Created `_reconcile_import_runs()` (line 416–465):
  ...
  - Called from `bootstrap()` after `_reconcile_reporting()` (line 726–727)
...
### Live DB Tests (BLOCKED — environment)

```
tests/test_u3b2_live_db_import_preview_validate.py — 14 errors (socket.gaierror)
tests/test_u1r1_bootstrap_completeness.py — 17 errors (socket.gaierror)
```

Both suites fail at setup with `socket.gaierror: [Errno 11001] getaddrinfo failed` —
Docker PostgreSQL is not running on this machine. This is a pre-existing
environment limitation, not a code defect. The same tests passed during S5-C
browser smoke when Docker PostgreSQL was available.

## Risk Assessment

- **GitNexus impact**: LOW — 4 upstream dependents, 2 direct callers
  (`main` in same file, `_ensure_tenant_tables` in `seed_test_tenant.py`).
  Neither caller is affected — CREATE TABLE IF NOT EXISTS is idempotent.
...
- **Product code impact**: None. The import API (`ImportService`) already has
  correct logic — this fix only ensures the table exists.
