...
> **R2 修正版（2026-08-24）**：父提交 R1=`48473ed`；分支 `zcode/dc12r1-mvp-l1-j1-h2-b-r2-r3-b0-r2-official-user-provisioning-truth-2026-08-24`。裁决目标更新为 `PASS_FOR_CTO_DC12R1_MVP_L1_J1_H2_B_R2_R3_B0_R2_PROTOCOL_TRUTH_REVIEW`。B0/R1 历史全部保留（R1 记录见下）。
>
> **R2 核心变更**：
> 1. **结论收窄（不撤回 R1 owner signup 唯一性）**：owner signup 不能产生双活跃注册（保留）；**但正式租户本地 `POST /api/v1/users` 可跨两租户创建同邮箱用户**——`email_exists` 仅查本租户 schema（`backend/crud/user.py:392-411`）、端点运行于租户作用域会话（`backend/api/v1/users.py:95-136`，挂载 `api/app.py:130`）、权限 `users:create` 属 admin 集（`backend/core/permission_registry.py:14,19`；owner 凭据路径引用 `backend/services/owner_credential_service.py:14`）、正式角色分配 `PUT /api/v1/users/{id}/roles`（`users.py:269`）、登录跨租户可见 `available_tenants`（`auth.py:286-292`）。
> 2. **M1 恢复 `BROWSER_WITH_OFFICIAL_API_PRECONDITION`**：W1/W2 owner 不同邮箱正式 signup → 各自经正式 API 创建同一规范化 admin 邮箱（必要时 PUT roles）→ 前置门验证登录精确见 W1/W2 → forgot/邮件链接/重置/旧密码拒绝/新密码双租户接受**全部浏览器 UI**；官方 API 仅限供给前置。
...
- 日期：2026-08-23（B0）；R1/R2 修正 2026-08-24（+08:00）
- 执行者：ZCode（B0 承接 OpenCode 文档与测试设计；R1/R2 证据真值修正。全程零运行/零产品修改）
- 分支（当前）：`zcode/dc12r1-mvp-l1-j1-h2-b-r2-r3-b0-r2-official-user-provisioning-truth-2026-08-24`（B0 原分支：`...b0-forgot-reset-browser-protocol-2026-08-23`；R1：`...b0-r1-provisioning-truth-2026-08-23`）
...
`PASS_FOR_CTO_DC12R1_MVP_L1_J1_H2_B_R2_R3_B0_R2_PROTOCOL_TRUTH_REVIEW`
...
| 节点总数 | 【R2 恢复】29 = browser 24（23 纯单副本 + M1 官方API前置浏览器节点）+ non-browser 5（F6/R6/M2/R13/RT0） |
...
- [x] 三文件精确 delta（相对父提交 48473ed，仅原三文件）
...
R2 发布 commit 即本台账所在提交；SHA 于 push 输出与 remote refs 可验（local==remote 断言即证明）。

## 下一步门禁

等待 CTO 同时接受：
1. OpenCode WSL literal zero-red 结果；
2. 本协议当前版本（B0→R1→R2 修正链）。

在两者均被接受前，不开始可执行 Playwright 实现，不启动运行时，不实施 H2-C。
