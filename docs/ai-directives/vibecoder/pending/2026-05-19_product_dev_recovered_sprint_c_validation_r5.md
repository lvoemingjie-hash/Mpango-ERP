...
Directive-ID: product-dev-recovered-sprint-c-validation-r5-2026-05-19
Priority: HIGH
Created: 2026-05-19
Status: pending
Target-Branch: product-dev-recovered
Target-Commit: c0f80d7cf69d197877453375974c067223cf366e
Validation-Scope: product-dev-recovered post Sprint C promotion validation (R5 — full product validation via v3.8)
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-05-19_product_dev_recovered_sprint_c_validation_r5.md

# Product Dev Recovered Sprint C Validation — R5

Objective:
Full product validation of `origin/product-dev-recovered` with actual test execution.

What changed in v3.8:
- Leo prompt now includes directive's "Required validation commands" section verbatim
- Git commands (preflight) + validation commands (product tests) executed in sequence
- Evidence format includes real test results (APP_IMPORT_SMOKE, RECEIVABLES_SUITE, etc.)
- Validation evidence gate: PASS_FOR_CTO_REVIEW rejected if validation results are "unknown"
- R4 only executed preflight git commands — R5 fixes this by sending actual test commands
...
R5 Acceptance Criteria (runner gates):
- GitHub Actions conclusion = success
- runner_name = mpango-lubuntu-01
- no gateway_timeout in Leo output
- fallbackUsed=false
- COMMANDS_EXECUTED=9/9 (5 preflight + 4 validation)
- APP_IMPORT_SMOKE != unknown
- RECEIVABLES_SUITE != unknown
- PHASE5_PAYMENT_REGRESSION != unknown
...
If R5 fails, report failure phase ONLY:
- script syntax / directive parsing / Leo timeout / evidence gate / validation gate / report push / final gate
