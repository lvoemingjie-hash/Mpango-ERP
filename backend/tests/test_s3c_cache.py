...
@pytest.mark.asyncio
async def test_close_redis_client_clears_client_from_closed_loop():
    """A stale client must not poison later cache operations."""
    stale_client = AsyncMock()
    stale_client.aclose.side_effect = RuntimeError("Event loop is closed")
    cache_module._redis_client = stale_client
