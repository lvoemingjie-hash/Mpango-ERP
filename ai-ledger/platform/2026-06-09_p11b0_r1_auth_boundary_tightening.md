...
**Date**: 2026-06-09
**Branch**: `codex/platform-p11b0-auth-transport-2026-06-08` (uncommitted R1 changes)
...
**New test class `TestBearerSuperAdminAccess`** (17 tests):
...
**New test class `TestBearerRealMiddleware`** (6 tests):
...
| `test_real_contextual_super_admin_denied` | Real JWT contextual super_admin → DENIED |
| `test_real_no_auth_denied` | No auth header → 401 |
...
| **P11-B0-R1 Bearer identity super_admin** | **17** | **✅ PASS** |
| **P11-B0-R1 Real middleware** | **6** | **✅ PASS** |
| P10-R1-B metadata redaction | 8 | ✅ PASS |
| **P10 total** | **137** | **✅ ALL PASS** |
| P0 regression (p0 + audit + audit_api + stats) | 71 | ✅ PASS |
| **Grand total** | **208** | **✅ ALL PASS** |
...
Acceptable because:
- Tests prove deny-by-default is preserved (17 new Bearer tests + 6 real middleware tests).
...
### ✅ Properly Guarded Endpoints

| Module | Auth Mechanism | Status |
|--------|---------------|--------|
| `api/v1/platform/p10/*` | `require_platform_operator` (P11-B0-R1) | ✅ Guarded |
| `api/v1/auth/*` | Public (login/refresh) or `get_current_user_context` | ✅ Correct |
| `api/v1/orders/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/users/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/roles/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/payments/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/inventory/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/skus/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/finance/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/wholesalers/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/invitations/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/retailers/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/dashboards/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/reports/*` | `RequirePermission` | ✅ Guarded |
| `api/v1/exports/*` (POST) | `RequirePermission("exports:create")` | ✅ Guarded |
| `api/v1/data_export/*` | `RequirePermission("exports:create")` | ✅ Guarded |
| `api/v1/client/*` | `resolve_client_identity` | ✅ Guarded |
| `api/v1/bi/assets/*` | `get_auth_context` + `get_tenant_context` (inline) | ✅ Guarded |
| `api/v1/metrics/*` (DELETE) | `RequirePermission("metrics:admin")` | ✅ Guarded |
| `api/v1/test/*` | `RequirePermission("system:admin")` + env gate | ✅ Guarded |

### ⚠️ Unauthenticated Endpoints (Intentional)
...
| `api/v1/metrics.py` (GET) | `GET /api/v1/metrics` | Requires auth via `get_current_user_context` ✅ |
| Root `/` | API info | Public metadata |

### 🚨 Known Auth Gap — OLD P0 Platform Endpoints (Pre-P10, NOT in R1 scope)
...
Uncommitted on branch `codex/platform-p11b0-auth-transport-2026-06-08`.
