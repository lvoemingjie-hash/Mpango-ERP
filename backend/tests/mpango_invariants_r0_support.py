...
BASELINE_MIGRATION_HEAD = "039_order_credit_holds"
...
async def seed_sku_with_stock(
    ...
    *,
    ...
    """Seed one tenant catalog product + SKU + stock row (+ optional price).
    ...
    Integration adaptation (2026-09-22): the current SKU identity requires a
    non-null catalog_product_id (models/sku.py) — a valid ACTIVE CatalogProduct
    row is seeded first so sellable-unit identity and the create-order route's
    catalog join (s JOIN catalog_products p ON p.is_active) are satisfied.
    """
    ...
    catalog_product = CatalogProduct(
        name=name,
        description="Invariants R0 synthetic catalog product",
        category="r0-fixtures",
        is_active=True,
    )
