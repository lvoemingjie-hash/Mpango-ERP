...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R7 status: evidence checkpoint, NO PASS verdict** — secret-argv removed (preflight reads DATABASE_URL/REDIS_URL from os.environ; a unique sentinel is proven absent from argv/log/output); setup_preflight.py hardened (strict env keys, exact DB scheme, blank-password reject, integer-valued ports, Compose-root dict check, neutral errors, Redis-creds reject); cross-host harness repaired (explicit Git Bash /usr/bin + /mingw64/bin, coreutils verify + fail-closed); direct preflight matrix 104/104 both orders; executable harness 20/20 both orders zero skip/xfail; complete H7 suite 218/218 both orders; real Compose JSON through the committed helper = OK. R5-R6 (`4746e180`) is superseded; the CTO cross-host reproduction is preserved as **187 collected / 174 passed / 13 failed** (closed by the R7 cross-host repair); the focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/VPS/native-setup.sh claim.
