...
Directive-ID: product-dev-recovered-sprint-c-validation-r6-2026-05-19
Priority: HIGH
Created: 2026-05-19
Status: pending
Target-Branch: product-dev-recovered
Target-Commit: c0f80d7cf69d197877453375974c067223cf366e
Validation-Scope: product-dev-recovered post Sprint C promotion validation (R6 — v3.9 section parser fix)
Allow-Code-Changes: false
Allow-Product-Push: false
Report branch: reports/lubuntu-validation
Report path: docs/ai-reports/lubuntu/2026-05-19_product_dev_recovered_sprint_c_validation_r6.md

# Product Dev Recovered Sprint C Validation — R6

Objective:
Full product validation of `origin/product-dev-recovered` with actual test execution.

What changed in v3.9:
- `extract_directive_section` now supports both "## Section Heading" and "Plain label:" formats
- `VALIDATION_CMD_COUNT` guaranteed single integer (no multi-line || echo 0 bug)
- R5 failed because plain-label sections ("Required validation commands:") were invisible to the parser
- R6 will correctly extract all 4 validation commands and send them to Leo
...
R6 Acceptance Criteria (runner gates):
...
- Final Gate passed (new workflow step validates REQUIRED_FIELDS in metadata)
