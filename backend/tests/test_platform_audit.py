...
class TestPlatformAuditLogModel:
    ...
    def test_no_updated_at(self):
        """Append-only: must NOT have updated_at."""
        assert not hasattr(PlatformAuditLog, 'updated_at')

    def test_no_is_deleted(self):
        """Append-only: must NOT have soft delete."""
        assert not hasattr(PlatformAuditLog, 'is_deleted')
