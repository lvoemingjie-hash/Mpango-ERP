...
**Accepted readiness-debt merge:** `a6ef3aac...` — DC-12R1-MVP-R0-R1 P2/P3 readiness-debt closure. The H7 dependency manifest reconciliation (`zcode/dc12r1-h7-bcrypt-manifest-reconciliation-2026-08-12`) branches from this tip and is the active pre-deployment prerequisite. **H7-R9 status: evidence checkpoint, NO PASS verdict** — R9 closes two bounded R8 defects without touching setup.sh, manifests, migrations, product code, Compose config, lockfiles or the Hypothesis test: (1) `_published_int` `re.match`→`.fullmatch` so a trailing-newline published string (`"5432\n"`) is no longer false-accepted (RED proven against `9f06d4a7`); `target` stays exact-int-only; (2) a direct test now proves the coreutils non-zero-return guard (`false` exe → `RuntimeError("coreutils probe failed")`), with mutation evidence that removing the guard turns it RED. Direct preflight matrix 129/129 both orders; executable harness 22/22 both orders zero skip/xfail; complete H7 suite 245/245 both orders in both file orderings; exact direct probes clean. R8 (`9f06d4a7`) is `SUPERSEDED_BY_H7_R9`; the CTO cross-host reproduction is preserved as **187 collected / 174 passed / 13 failed**; the focused regression retains one environment-gated unresolved Hypothesis `HealthCheck.too_slow` red node; verdict is `STOP_AND_REPORT_CTO_AWAITING_KILO_AND_LUBUNTU_ZERO_RED`. No deployment/Playwright/VPS/native-Linux-success/merge claim.
