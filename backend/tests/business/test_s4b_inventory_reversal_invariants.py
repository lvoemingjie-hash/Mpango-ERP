"""S4-B1 audit tests for return/cancel inventory reversal invariants.

This file intentionally does not modify production behavior. Expected gaps are
encoded as strict xfail tests so the audit exposes what S4-B2 must implement.
"""
...
RETURN_RESTOCK_GAP = (
    "S4-B2 required: return_order transitions and posts refund ledger, but does "
    "not restore inventory or write restock InventoryMovement."
)
...
@pytest.mark.asyncio
@pytest.mark.xfail(strict=True, reason=RETURN_RESTOCK_GAP)
...
@pytest.mark.asyncio
async def test_return_posts_refund_ledger_but_no_inventory_restock_yet(async_session):
    await _prepare_inventory_schema(async_session)
    sku, order = await _create_fulfilled_order_with_deducted_stock(
